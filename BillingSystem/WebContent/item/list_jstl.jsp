...
<div class="container-fluid">
	...
	<div class="row">	
							...
							<tbody>
								<c:forEach var="item" items="${sessionScope.itemList}">
									<c:if test="${item.code != 'CCOFFEE' }">
										<tr>
											<td><a
												href="${pageContext.request.contextPath}/ManageItemController?action=detail&id=${item.id}">${item.id}</a></td>
											<td><img
		...
		<c:if test="${item != null && from != 'orderList'}">
			<div class="col-md-6">
	...
	<div class="row">
		<div class=col alert alert-success>
			Order #<a
				href="${pageContext.request.contextPath}/OrderController?action=orderDetail&orderId=${orderId}">${orderId}</a>
			${msg}
		</div>
	</div>
	<c:if test="${sessionScope.orderItemList.size() > 0 && from != 'orderList'}">
		<div class="row">
			<div class="col-md-6">
